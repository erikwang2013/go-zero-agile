...
var (
    ...
    roleRowsExpectAutoSet   = strings.Join(stringx.Remove(roleFieldNames, "`create_time`", "`update_time`", "`create_at`", "`update_at`"), ",")
...
func (m *defaultRoleModel) Insert(ctx context.Context, data *Role) (sql.Result, error) {
    ...
    ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
        ...
        return conn.ExecCtx(ctx, query, data.Id, data.ParentId, data.Name, data.Info, data.Code, data.Status, data.IsDelete)
    }, roleIdKey)
    return ret, err
}
...
func (m *defaultRoleModel) Update(ctx context.Context, data *Role) error {
    ...
    _, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
        ...
        return conn.ExecCtx(ctx, query, data.ParentId, data.Name, data.Info, data.Code, data.Status, data.IsDelete, data.Id)
    }, roleIdKey)
    return err
}
